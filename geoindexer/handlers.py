...
class Container:
    ...
    def get_props(self) -> dict:
        ...
        if ext in {'gdb', 'gpkg', 'db'}:
            ...
            layers = fiona.listlayers(self.container)
            progress = tqdm(
                layers,
                desc=f"Indexing layers in {Path(self.container).name}",
                unit="layer",
                disable=not logger.isEnabledFor(logging.DEBUG)
            )

            for layer_name in progress:
                try:
    ...
    def batch(image_paths: list[str], max_workers: int = 8) -> list[dict]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            ...
            for f in tqdm(as_completed(futures),
                          total=len(futures),
                          desc="Extracting GPS data from images...",
                          unit="img",
                          disable=not logger.isEnabledFor(logging.DEBUG)):
                result = f.result()
                if result:
...
class Lidar:
    ...
    warned_once = False  # suppress repeated warnings
    ...
    def _run_pdal(self) -> dict | None:
        ...
        if not self.pdal_available:
            if not Lidar.warned_once:
                logger.warning("PDAL is not found on the system. Lidar indexing will be skipped."
                               "To enable Lidar support, install PDAL via conda: `conda install -c conda-forge pdal`"
                )
                Lidar.warned_once = True
            return None
